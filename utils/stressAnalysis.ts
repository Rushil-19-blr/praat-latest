...
const ENHANCED_WEIGHTS = { jitter: 2.5, shimmer: 2.5, hnr: 2.0, f0Mean: 1.8, f0Range: 1.3, speechRate: 1.6, f1: 0.4, f2: 0.4, interactions: 1.2 };
...
const calculatePopulationScores = (values: MeasureValues, profile: VocalProfile) => {
    ...
    const calculateHNRScore = (hnr: number): number => { 
        if (hnr >= HNR_THRESHOLDS.excellent) return 0; 
        if (hnr >= HNR_THRESHOLDS.good) { 
            const deficit = HNR_THRESHOLDS.excellent - hnr; 
            return 2 * (deficit / (HNR_THRESHOLDS.excellent - HNR_THRESHOLDS.good)); 
        } 
        if (hnr >= HNR_THRESHOLDS.moderate) { 
            const deficit = HNR_THRESHOLDS.good - hnr; 
            return 2 + (4 * Math.pow(deficit / (HNR_THRESHOLDS.good - HNR_THRESHOLDS.moderate), 1.3)); 
        } 
        const deficit = Math.max(0, HNR_THRESHOLDS.moderate - hnr); 
        return 6 + (4 * Math.min(1, deficit / (HNR_THRESHOLDS.moderate - HNR_THRESHOLDS.poor))); 
    };
...
const calculateStressFromPopulation = (values: MeasureValues, profileType: ProfileType): StressResultType => {
  ...
  const calculateInteractionEffects = (scores: { [key: string]: number }): number => { 
      ...
      if (scores.hnr > 5 && scores.shimmer > 4) bonus += 0.35 * Math.min(scores.hnr, scores.shimmer); 
      return bonus; 
  };
  ...
  const finalScore = Math.min(100, (totalWeightedScore / totalWeight) * 10);

  if (finalScore >= 60) return { level: StressLevel.High, score: finalScore, explanation: 'High stress detected based on significant deviation from population norms. Vocal markers indicate acute physiological arousal or strain.' };
  if (finalScore >= 30) return { level: StressLevel.Medium, score: finalScore, explanation: 'Medium stress indicated. Some vocal parameters show moderate deviation from typical patterns, suggesting heightened cognitive load or emotional engagement.' };
  return { level: StressLevel.Low, score: finalScore, explanation: 'Low stress detected. Vocal biomarkers are within normal ranges for the selected profile, reflecting a calm physiological state.' };
};
...
const BASELINE_WEIGHTS = { jitter: 2.8, shimmer: 2.8, hnr: 2.5, f0Mean: 2.2, f0Range: 1.8, speechRate: 2.0, interactions: 1.5 };
...
const calculateDeviationScores = (current: MeasureValues, baseline: MeasureValues) => {
    ...
    const hnrRatio = safeRatio(current.hnr, baseline.hnr);
    ...
    const scores = {
        jitter: 0, shimmer: 0, hnr: 0, f0Mean: 0, f0Range: 0, speechRate: 0,
...
const generateBaselineExplanation = (level: StressLevel, scores: any): { explanation: string; stressType?: string } => {
    ...
    if (scores.hnr > 3) issues.push(`voice clarity dropped`);
